...
<Tip>
  Vision prefix: control the optional `ActiveTokenSampler` with `--policy.use_grid_token_sampler` (default `true`) and the number of sampled vision tokens with `--policy.grid_token_sampler_num_tokens` (default `16`). These are saved in `config.json`; when loading a pretrained checkpoint, keep them consistent with how that model was trained.
</Tip>
