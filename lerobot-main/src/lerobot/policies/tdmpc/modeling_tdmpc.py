...
class TDMPCObservationEncoder(nn.Module):
    ...
    def __init__(self, config: TDMPCConfig):
        ...
        if config.image_features:
            ...
            out_shape = get_output_shape(conv_block, dummy_shape)[1:]
            c_out = out_shape[0]

            if config.use_vision_grid_token_prune:
                self._use_image_prune = True
                self.image_enc_convs = conv_block
                self.vision_token_sampler = ActiveTokenSampler(
                    vision_dim=c_out,
                    num_tokens=config.vision_grid_token_prune_num_tokens,
                    embed_coords=True,
                )
                flat_dim = config.vision_grid_token_prune_num_tokens * c_out
                self.image_enc_prune_tail = nn.Sequential(
                    nn.Linear(flat_dim, config.latent_dim),
                    nn.LayerNorm(config.latent_dim),
                    nn.Sigmoid(),
                )
                self.image_enc_layers = None
            else:
                self._use_image_prune = False
                self.image_enc_convs = None
                self.vision_token_sampler = None
                self.image_enc_prune_tail = None
    ...
    def forward(self, obs_dict: dict[str, Tensor]) -> Tensor:
        ...
        # NOTE: Order of observations matters here.
        if self.config.image_features:
            if getattr(self, "_use_image_prune", False):

                def _encode_img(x: Tensor) -> Tensor:
                    h = self.image_enc_convs(x)
                    z = self.vision_token_sampler(h).flatten(1)
                    return self.image_enc_prune_tail(z)

                feat.append(
                    flatten_forward_unflatten(
                        _encode_img, obs_dict[next(iter(self.config.image_features))]
                    )
                )
            else:
                feat.append(
