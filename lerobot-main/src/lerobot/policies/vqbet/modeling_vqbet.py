...
class VQBeTRgbEncoder(nn.Module):
    ...
    def __init__(self, config: VQBeTConfig):
        ...
        self.use_vision_prune = config.use_vision_grid_token_prune
        if self.use_vision_prune:
            self.vision_token_sampler = ActiveTokenSampler(
                vision_dim=in_channels,
                num_tokens=config.vision_grid_token_prune_num_tokens,
                embed_coords=True,
            )
            flat_dim = config.vision_grid_token_prune_num_tokens * in_channels
            mid_dim = config.spatial_softmax_num_keypoints * 2
            self.prune_to_spatial = nn.Linear(flat_dim, mid_dim)
            self.pool = None
        else:
            self.pool = SpatialSoftmax(feature_map_shape, num_kp=config.spatial_softmax_num_keypoints)
            self.vision_token_sampler = None
            self.prune_to_spatial = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        h = self.backbone(x)
        if self.use_vision_prune:
            tok = self.vision_token_sampler(h)
            x = self.relu(self.out(self.prune_to_spatial(tok.flatten(1))))
        else:
            x = torch.flatten(self.pool(h), start_dim=1)
