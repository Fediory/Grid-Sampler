...
from lerobot.policies.active_token_sampler import ActiveTokenSampler  # noqa: F401
...
class PI0FlowMatching(nn.Module):
    ...
    # === 🛡️ 终极防御性补丁 (Ultimate Patch) ===
        # print("DEBUG: 正在检查 Embedding 维度...")
        new_embs = []
        for i, e in enumerate(embs):
            # print(f"  -> Tensor {i} 原始形状: {e.shape}")
            
            # 情况1: 只有2维 [L, D] -> 补成 [1, L, D]
            if e.ndim == 2:
                e = e.unsqueeze(0)
            
            # 情况2: 3维但第一维是 L (不是1) -> 转置成 [1, L, D]
            # 假设 Batch Size 应该是 1 (Inference 模式)
            elif e.ndim == 3 and e.shape[0] != 1 and e.shape[1] == 1:
                e = e.permute(1, 0, 2)
            
            # print(f"     修正后形状: {e.shape}")
            new_embs.append(e)
